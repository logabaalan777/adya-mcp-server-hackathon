...
class GetPerformanceToolHandler(ToolHandler):
    def __init__(self):
        super().__init__("get_alacritty_performance")

    def get_tool_description(self) -> Tool:
        return Tool(
            name=self.name,
            description="Get performance-related settings (render timer, vsync, etc.) from the Alacritty config (YAML or TOML).",
            inputSchema={
                "type": "object",
                "properties": {
                    "config_path": {
    ...
    async def run_tool(self, args: dict) -> list[TextContent]:
        ...
        try:
            ...
            if ext in [".yml", ".yaml"]:
                ...
                performance_text = yaml.safe_dump(performance)
