...
async def client_and_server_execution(payload: Dict[str, Any], streaming_callback: Optional[Any] = None) -> ClientAndServerExecutionResponse:
    try:
        ...
        #     # Initial LLM call
        #     api_key = selected_server_credentials.get("MCP-CLAUDE", {}).get("api_key")
        #     if api_key:
        #         client_details["api_key"] = api_key
        #     if "arguments" in client_details:
        #         client_details.update(client_details["arguments"])
        #     tool_name = client_details.get("tool_name")
        #     if tool_name and tool_name in [t["function"]["name"] for t in client_details.get("tools", [])] or tool_name in ["get_conversation_history", "clear_conversation", "get_model_info"]:
        #         # Direct tool call, do NOT route through LLM
        ...
        #         result = ClientAndServerExecutionResponse()
        #         result.Data["messages"] = [json.dumps(tool_result, indent=2) if not isinstance(tool_result, str) else tool_result]
        #         result.Status = True
        #         return result
        ...
        #         # Ensure output is wrapped properly
        #         if isinstance(tool_output, str):
        #             tool_result = TextContent(type="text", text=tool_output)
        #         elif isinstance(tool_output, dict) and "text" in tool_output:
        #             tool_result = TextContent(type="text", text=tool_output["text"])
        #         elif isinstance(tool_output, list):
        #             # Assume already contains TextContent
        #             messages = tool_output
        #         else:
        #             # fallback
        #             tool_result = TextContent(type="text", text=str(tool_output))

        #         result = ClientAndServerExecutionResponse()
        #         result.Data["messages"] = [tool_result.dict()]  # must be list of TextContent
        #         result.Status = True
        #         return result
        elif selected_client == "MCP_CLIENT_GEMINI":
            tool_name = client_details.get("tool_name")
        ...
        elif selected_client == "MCP_CLIENT_LOOKER":
            tool_name = client_details.get("tool_name")
            arguments = client_details.get("arguments", {})
            tool_result = await call_and_execute_tool(selected_server, selected_server_credentials, tool_name, arguments)
            result = ClientAndServerExecutionResponse()
